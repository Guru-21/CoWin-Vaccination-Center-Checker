...
def run():
    ...
    #AREA PIN
    area_pin = st.text_input('Enter your Area Pin-Code Eg.100023')
    ...
    if st.button("Search"):
        try:
            ...
            response = requests.get(f"https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByPin?pincode={area_pin}&date={new_date}",
                headers=header)
            data = response.json()
            centers = pd.DataFrame(data.get('centers'))
            if centers.empty:
                st.warning("No centres found for " + new_date + ' in ' + str(area_pin) + ' Please Check Later!')
            else:
                session_ids = []
                for j, row in centers.iterrows():
                    ...
                    print(new_df)
                    new_df.columns = ['Center_ID', 'Name', 'Address', 'Fee','Availability', 'Minimum Age', 'Vaccine Type', 'Timing']
                    new_df = new_df[['Center_ID', 'Name', 'Fee','Availability', 'Minimum Age', 'Vaccine Type', 'Timing', 'Address']]
